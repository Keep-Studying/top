...
/**
 * 对 ThreadPoolExecutor 类进行注释说明
 *
 * todo：拒绝策略相关是有报错的，由于把ThreadPoolExecutor完全拷贝出来进行注释，
 */
/*
...
public class ThreadPoolExecutor extends AbstractExecutorService {
    /**
     * ctl是一个AtomicInteger类型的变量，是对线程池的运行状态和线程池中有效
     * 线程的数量进行控制的一个字段，它包含两部分的信息：
     * - runState：线程池的运行状态，用ctl的高3位来保存
     * - workerCount：线程池内有效的线程数量，用ctl的低29位来保存
     *
    ...
    /**
     * 它是ThreadFactory类型的变量，用来创建线程。
     * 默认使用Executors.defaultThreadFactory()来创建线程。使用默认的
     * ThreadFactory来创建线程时，会使新创建的线程具有相同的NORM_PRIORITY
    ...
    /**
     * 线程池维护线程所允许的空闲时间。
     * 当线程池中的线程数量大于corePoolSize的时候，如果这时没有新的任务
     * 提交，核心线程外的线程不会立即销毁，而是会等待，直到等待的时间超过
     * 了keepAliveTime
     *
    ...
    /**
     * 线程池中允许的最大线程数，如果当前阻塞队列满了，且继续提交任务，则创建新
     * 的线程执行任务，前提是当前线程数小于 maximumPoolSize
    ...
    final void reject(Runnable command) {
        //todo 为了防止报错，注释掉该行代码，可参考jdk源码进行查看
    ...
    /**
     * addWorker方法主要是在线程池中创建一个新的线程并执行
     * -firstTask参数用于指定新增的线程执行的第一个任务，
     * -core参数：
     *  -为true表示在新增线程时会判断当前线程数是否少于corePoolSize，
     *  -false表示新增线程前需要判断当前活动线程数是否小于maxinumPoolSize
    ...
    private boolean addWorker(Runnable firstTask, boolean core) {
        ...
        try {
            // 根据firstTask来创建Worker对象，线程池维护的线程其实就是一个个的worker对象
    ...
    final void runWorker(ThreadPoolExecutor.Worker w) {
        ...
        w.firstTask = null;
        //允许中断，
        //将构造worker时的  @code{ setState(-1);} 的state改为0
    ...
    /**
     * submit方法中只是对任务进行了包装，最终也会调用到execute方法，
     * 因此分析execute方法即可
    ...
    public void execute(Runnable command) {
         ...
         */
        //获取ctl，ctl记录着runState和workerCount
        int c = ctl.get();
        /*
    ...
    //todo 4种拒绝策略，为了防止报错，均重写了rejectedExecution方法，可查看jdk源码看原来内容
