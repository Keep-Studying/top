 ...
 *
 * 计算空间：假设我们有N{2，3，64}个数，每个数字的范围是0~2亿，则最大的数字为2亿，即Max=2亿，
 * 假设下述内容都用int数组来保存
 * 如果按照年龄问题，则需要开2亿个数组空间，2亿*4（byte）/1024/1024≈762M，即单单这个数组就
 * 需要这么多的内存（超过了问题中给出的机器500M）
 * 而如果用bitMap，则将上述结果除以32即可，因为1 int = 32bit，2亿*4（byte）/32/1024/1024≈23M
 * （该数组所用空间远远小于上述762M）
 *
 * 应用：
 * 1.数据判重
 * 2.对没有重复的数据进行排序
 * 3.根据1和2可以扩展出很多其他的应用，比如找不重复的问题，统计数据，打标签问题等
 *
 * 缺点：
 * 1.数据不能重复，数据只有0和1，也就是有或者没有，不知道有多个
 * 2.数据量少时，相对于普通的hash没有优势
 * 3.无法处理字符串：hash冲突
 * @author study
